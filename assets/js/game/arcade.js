class LisarArcade2D {
  ...
  spawnEntity(dt) {
    ...
    if (this.randomBlockTimer >= 1.4 && this.gameTimer < 165) {
      ...
      if (Math.random() < 0.38) {
        ...
        if (noObstaclesNear && noPowerupsNear) {
          ...
          // centerY(obstáculo_superior) = 238.75 (Centro exacto del rectángulo verde de referencia)
          const platHeight = 35;
          const platY = 238.75 - (platHeight / 2); // Y = 221.25 (Centro visual = 238.75)
    ...
    if (challengeIndex === 0) {
      // 0. PLATAFORMA SUELO: Bloque terrestre de altura baja (width: 320, height: 70) + monedas arriba
      const cubeY = 310; // Y = 310 en el suelo (monedas a Y = 250 en la franja verde)
      this.enemies.push({ type: 0, x: startX, y: cubeY, width: 320, height: 70, vx: -this.floorSpeed, hp: 9999 });
      for (let i = 0; i < 6; i++) {
        this.coins.push({ x: startX + 20 + i * 45, y: 250, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    } else if (challengeIndex === 1) {
      // 1. HAZARD VOLADOR: Robot OVNI volador centrado en el bloque verde + arco de monedas central
      this.spawnEnemy1(progress);
      for (let i = 0; i < 5; i++) {
        const archY = 220 - Math.sin((i / 4) * Math.PI) * 20; // Y entre 200 y 220
        this.coins.push({ x: startX + i * 48, y: archY, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    } else if (challengeIndex === 2) {
      // 2. ENEMIGO RODANTE: Bola eléctrica en el suelo + rombo de monedas en el bloque verde
      this.spawnEnemy2(progress);
      const highY = 225;
      const diamondOffsets = [{ dx: 0, dy: 0 }, { dx: 36, dy: -12 }, { dx: 36, dy: 12 }, { dx: 72, dy: 0 }];
      ...
      // 3. REABASTECIMIENTO: Powerup centrado en el bloque verde + onda de monedas
      const startY = 225;
      ...
      for (let i = 0; i < 6; i++) {
        this.coins.push({ x: startX + i * 50, y: startY + Math.sin(i * 0.9) * 16, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    } else if (challengeIndex === 4) {
      // 4. PLATAFORMA SUPERIOR: centerY = 238.75 (Centro exacto del rectángulo verde)
      const platHeight = 35;
      const platY = 238.75 - (platHeight / 2); // Y = 221.25
      this.enemies.push({ type: 0, x: startX, y: platY, width: 300, height: platHeight, vx: -this.floorSpeed, hp: 9999 });
      for (let i = 0; i < 6; i++) {
        this.coins.push({ x: startX + 20 + i * 46, y: platY - 55, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    } else if (challengeIndex === 5) {
      // 5. DOBLE AMENAZA: Robot OVNI en el centro + rodante en el suelo + monedas intermedias
      this.spawnEnemy1(progress);
      ...
      // 6. PARKOUR DE CUBOS: Dos plataformas bajas en el suelo con monedas arriba
      const cubeY = 310; // Y = 310 en el suelo
      this.enemies.push({ type: 0, x: startX, y: cubeY, width: 120, height: 70, vx: -this.floorSpeed, hp: 9999 });
      this.enemies.push({ type: 0, x: startX + 200, y: cubeY, width: 120, height: 70, vx: -this.floorSpeed, hp: 9999 });
      for (let i = 0; i < 6; i++) {
        this.coins.push({ x: startX + i * 52, y: 250, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    } else if (challengeIndex === 7) {
      // 7. COIN RUSH: Doble hélice de monedas centrada en el bloque verde (Y = 225)
      const centerY = 225;
      this.powerups.push({ x: startX + 120, y: centerY, width: 50, height: 50, vx: -this.floorSpeed, isBoot: Math.random() > 0.5, frameTimer: 0 });
      for (let i = 0; i < 8; i++) {
        this.coins.push({ x: startX + i * 40, y: centerY + Math.sin(i * 0.8) * 22, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
        this.coins.push({ x: startX + i * 40, y: centerY - Math.sin(i * 0.8) * 22, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    } else if (challengeIndex === 8) {
      // 8. PLATAFORMAS SUPERIORES: centerY = 238.75 (Centro exacto del rectángulo verde)
      const platHeight = 35;
      const platY = 238.75 - (platHeight / 2); // Y = 221.25
      this.enemies.push({ type: 0, x: startX, y: platY, width: 140, height: platHeight, vx: -this.floorSpeed, hp: 9999 });
      this.enemies.push({ type: 0, x: startX + 180, y: platY, width: 140, height: platHeight, vx: -this.floorSpeed, hp: 9999 });
      for (let i = 0; i < 3; i++) {
        this.coins.push({ x: startX + 15 + i * 42, y: platY - 55, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
        this.coins.push({ x: startX + 195 + i * 42, y: platY - 55, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    } else if (challengeIndex === 9) {
      // 9. PLATAFORMA SUPERIOR: centerY = 238.75 (Centro exacto del rectángulo verde)
      const platHeight = 35;
      const platY = 238.75 - (platHeight / 2); // Y = 221.25
      this.enemies.push({ type: 0, x: startX, y: platY, width: 240, height: platHeight, vx: -this.floorSpeed, hp: 9999 });
      this.spawnEnemy2(progress, 60);
      for (let i = 0; i < 5; i++) {
        this.coins.push({ x: startX + 20 + i * 42, y: platY - 55, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    }
  }
  ...
  spawnEnemy1(progress, offsetX = 0) {
    // Enemigo volador robot OVNI ubicado EXACTAMENTE en la franja central del bloque verde (Y = 195 a 215)
    const spawnY = 195 + Math.random() * 20;
    this.enemies.push({
      type: 1,
      x: this.logicalWidth + 30 + offsetX,
      y: spawnY,
  ...
  update(dt) {
    ...
    // ── PLAYING PATH ─────────────────────────────────────────────────────────────
    // SAFEGUARD TOTAL: Eliminar cualquier bloque (Y < 190), moneda (Y < 150) o enemigo volador (Y < 170) que intente subir a la zona del HUD
    this.enemies = this.enemies.filter(e => (e.type !== 0 || e.y >= 190) && (e.type !== 1 || e.y >= 170));
    this.coins = this.coins.filter(c => c.y >= 150);
  ...
  draw(dt) {
    ...
    this.coins.forEach(c => {
      if (c.y < 150) return; // NUNCA dibujar monedas por encima de Y = 150
