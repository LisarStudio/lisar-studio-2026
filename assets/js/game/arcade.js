class LisarArcade2D {
  ...
  spawnEntity(dt) {
    ...
    if (this.gameTimer >= 20.0) {
      ...
      letterSchedule.forEach(item => {
        if (this.gameTimer >= item.time && !this.spawnedLetters[item.letter]) {
          ...
          const randomY = 240 + Math.random() * 45; // Y entre 240 y 285
    ...
    if (this.randomBlockTimer >= 1.4 && this.gameTimer < 165) {
      ...
      if (Math.random() < 0.38) {
        ...
        if (noObstaclesNear && noPowerupsNear) {
          ...
          const randomY = 275 + Math.random() * 20; // Y entre 275 y 295 (safe from HUD and exactly in the green block height range)
    ...
    if (challengeIndex === 0) {
      // 0. PLATAFORMA SUELO LARGA: Bloque horizontal terrestre ancho (width: 320) + 6 monedas arriba (bajos)
      const cubeY = this.logicalHeight - 100 - 70; // Y = 280 (coins at 220)
      this.enemies.push({ type: 0, x: startX, y: cubeY, width: 320, height: 100, vx: -this.floorSpeed, hp: 9999 });
      for (let i = 0; i < 6; i++) {
      ...
      // 1. HAZARD VOLADOR: Enemigo volador + arco parabólico de monedas (bajo)
      this.spawnEnemy1(progress);
      for (let i = 0; i < 5; i++) {
        const archY = 250 - Math.sin((i / 4) * Math.PI) * 25; // Y entre 225 y 250
        this.coins.push({ x: startX + i * 48, y: archY, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    } else if (challengeIndex === 2) {
      // 2. ENEMIGO RODANTE: Bola eléctrica rodante + rombo de monedas (bajo)
      ...
      // 3. REABASTECIMIENTO: Magnetic Boot (si está herido) o carga de Lu + onda de monedas (bajo)
      const startY = 255;
      ...
      // 4. PLATAFORMA FLOTANTE LARGA EN EL CENTRO: Bloque horizontal de centro ancho (width: 320) + monedas arriba (bajo)
      const randomY = 280; // Y = 280 (coins at 220)
      ...
      // 5. DOBLE AMENAZA: Un enemigo volador alto, uno rodante bajo, y monedas en zigzag intermedio (bajo)
      this.spawnEnemy1(progress);
      ...
      // 6. PARKOUR DE CUBOS: Dos plataformas seguidas en el suelo con monedas flotando arriba (bajo)
      const cubeY = this.logicalHeight - 100 - 70; // Y = 280
      this.enemies.push({ type: 0, x: startX, y: cubeY, width: 120, height: 100, vx: -this.floorSpeed, hp: 9999 });
      this.enemies.push({ type: 0, x: startX + 200, y: cubeY, width: 120, height: 100, vx: -this.floorSpeed, hp: 9999 });
      for (let i = 0; i < 6; i++) {
        this.coins.push({ x: startX + i * 52, y: cubeY - 60, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 }); // Coins at 220
      }
    } else if (challengeIndex === 7) {
      // 7. COIN RUSH: Sin enemigos, gran oleada de monedas en doble hélice + rayito de salud (bajo)
      const centerY = 265;
      ...
      // 8. PASOS ESCALONADOS: Dos plataformas flotantes a diferente altura media, monedas sobre ellas (bajo)
      this.enemies.push({ type: 0, x: startX, y: 295, width: 150, height: 35, vx: -this.floorSpeed, hp: 9999 });
      this.enemies.push({ type: 0, x: startX + 190, y: 280, width: 150, height: 35, vx: -this.floorSpeed, hp: 9999 });
      for (let i = 0; i < 3; i++) {
        this.coins.push({ x: startX + 15 + i * 45, y: 295 - 60, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 }); // Coins at 235
        this.coins.push({ x: startX + 205 + i * 45, y: 280 - 60, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 }); // Coins at 220
      }
    } else if (challengeIndex === 9) {
      // 9. PLATAFORMA FLOTANTE INTERMEDIA: Plataforma flotante en medio + monedas arriba + enemigo rodante abajo! (bajo)
      const platformY = 280; // Y = 280
      this.enemies.push({ type: 0, x: startX, y: platformY, width: 260, height: 35, vx: -this.floorSpeed, hp: 9999 });
  ...
  spawnEnemy1(progress, offsetX = 0) {
    // Enemigos voladores únicamente en la franja de altura del bloque verde (250px - 290px)
    const minY = 250;
    const maxY = 290;
  ...
  update(dt) {
    ...
    if (this.player.hp <= 60 && !this.luBoostActive && (this.luBoostCooldown || 0) <= 0) {
      ...
      const playableTopY = 185 / this.scale;
      this.luBoostY = 255; // Y = 255 (fixed height at green block level)
