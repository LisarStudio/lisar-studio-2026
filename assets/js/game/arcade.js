class LisarArcade2D {
  ...
  spawnEntity(dt) {
    ...
    if (this.randomBlockTimer >= 1.4 && this.gameTimer < 165) {
      ...
      if (Math.random() < 0.38) {
        ...
        if (noObstaclesNear && noPowerupsNear) {
          ...
          const randomY = 225 + Math.random() * 15; // Y = 225 a 240 (EXACTAMENTE en el centro del bloque verde)
    ...
    if (challengeIndex === 0) {
      ...
      // 4. PLATAFORMA FLOTANTE LARGA (EXACTA AL BLOQUE VERDE): width: 300, height: 35 en Y = 230
      const randomY = 230; // Y = 230 (centro exacto del bloque verde)
      this.enemies.push({ type: 0, x: startX, y: randomY, width: 300, height: 35, vx: -this.floorSpeed, hp: 9999 });
      for (let i = 0; i < 6; i++) {
        this.coins.push({ x: startX + 20 + i * 46, y: randomY - 55, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
      ...
      // 8. PASOS ESCALONADOS: Dos plataformas flotantes a la altura del bloque verde (Y = 245 y Y = 220)
      this.enemies.push({ type: 0, x: startX, y: 245, width: 140, height: 35, vx: -this.floorSpeed, hp: 9999 });
      this.enemies.push({ type: 0, x: startX + 180, y: 220, width: 140, height: 35, vx: -this.floorSpeed, hp: 9999 });
      for (let i = 0; i < 3; i++) {
        this.coins.push({ x: startX + 15 + i * 42, y: 245 - 55, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
        this.coins.push({ x: startX + 195 + i * 42, y: 220 - 55, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    } else if (challengeIndex === 9) {
      // 9. PLATAFORMA FLOTANTE INTERMEDIA: Plataforma en Y = 230 + monedas arriba + enemigo rodando abajo
      const platformY = 230; // Y = 230
      this.enemies.push({ type: 0, x: startX, y: platformY, width: 240, height: 35, vx: -this.floorSpeed, hp: 9999 });
      this.spawnEnemy2(progress, 60);
      for (let i = 0; i < 5; i++) {
        this.coins.push({ x: startX + 20 + i * 42, y: platformY - 55, width: 60, height: 60, vx: -this.floorSpeed, frame: 0, frameTimer: 0 });
      }
    }
  }
